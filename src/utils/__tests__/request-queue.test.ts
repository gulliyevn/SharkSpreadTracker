...
describe('request-queue', () => {
  ...
  describe('add', () => {
    ...
    it('should handle request errors', async () => {
      ...
      await expect(requestQueue.add(requestFn)).rejects.toThrow(
        'Request failed'
      );
    });
    ...
    it('should respect priority order', async () => {
      ...
      const lowPriority = vi.fn().mockImplementation(async () => {
        results.push(1);
        return 'low';
      });

      const highPriority = vi.fn().mockImplementation(async () => {
        results.push(2);
        return 'high';
      });

      // Добавляем сначала LOW, потом HIGH
      const promise1 = requestQueue.add(lowPriority, {
        priority: RequestPriority.LOW,
      });
      const promise2 = requestQueue.add(highPriority, {
